...
sudo apt-get -y -q install \
  ...
  postgresql-contrib
# Post-installation
echo "PostgreSQL: updating /etc/postgresql/9.5/main/postgresql.conf"
sudo sed -i "s/#listen_address.*/listen_addresses '*'/" /etc/postgresql/9.5/main/postgresql.conf
echo "Enabled listen_addresses '*'"
echo "PostgreSQL: updating /etc/postgresql/9.5/main/pg_hba.conf"
sudo sh -c 'cat >> /etc/postgresql/9.5/main/pg_hba.conf <<EOF
